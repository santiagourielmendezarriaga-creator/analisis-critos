    ...
    if os.path.exists(STATE_FILE):
        try:
            with open(STATE_FILE, "r") as f:
                data = json.load(f)
            # Asegurar todas las claves necesarias
            data.setdefault("price_history", {})
            data.setdefault("last_price", {})
            data.setdefault("last_entry_price", {})
            data.setdefault("highest_price", {})
            data.setdefault("last_action", {})
            data.setdefault("trades", [])
            data.setdefault("daily_pnl", 0.0)
            # Garantizar que price_history tenga BTC y ETH como deques
            for sym in ["BTC", "ETH"]:
...
def can_trade(state, daily_loss_limit):
    ...
    if state["daily_pnl"] <= -daily_loss_limit:
        return False, f"Pérdida diaria máxima (${-daily_loss_limit:.2f})"
    return state["daily_trades"] < MAX_DAILY_TRADES, ""

def execute_trade(symbol, action, price, state, commission_pct, slippage_pct, pos_size_pct, daily_loss_limit):
    can, reason = can_trade(state, daily_loss_limit)
    if not can:
        return None, f"❌ {reason}"
    if action == "BUY":
        amount = state["balance"] * pos_size_pct
        if amount <= 0:
...
# ==================== INICIALIZAR ESTADO (CON DEQUES) ====================
saved = load_state()
if saved:
    state = saved
    # Reconstruir deques con longitud máxima 100
    state["price_history"] = {sym: deque(state["price_history"].get(sym, []), maxlen=100) for sym in ["BTC", "ETH"]}
...
if strategy == "change":
    ...
    oversold = st.sidebar.number_input("RSI sobreventa", 20, 40, DEFAULT_RSI_OVERSOLD)
    overbought = st.sidebar.number_input("RSI sobrecompra", 60, 80, DEFAULT_RSI_OVERBOUGHT)
else:
    ma_fast = st.sidebar.number_input("MA rápida", 3, 20, DEFAULT_MA_FAST)
    ma_slow = st.sidebar.number_input("MA lenta", 10, 50, DEFAULT_MA_SLOW)
...
pos_size_pct = st.sidebar.slider("Tamaño posición (% saldo)", 0.1, 1.0, DEFAULT_POSITION_SIZE_PCT, 0.05)
...
st.info("✅ Estrategia configurable, stop-loss, trailing, take-profit, límite pérdida diaria, tamaño posición porcentual. Backtesting abajo.")
...
for sym, price in [("BTC", btc_p), ("ETH", eth_p)]:
    ...
    if strategy == "change":
        signal, _ = get_signal_change(hist, change_th)
    elif strategy == "rsi":
        signal, _ = get_signal_rsi(hist, oversold, overbought)
    else:
        signal, _ = get_signal_ma(hist, ma_fast, ma_slow)
...
# ==================== BACKTESTING ====================
with st.expander("📊 Backtesting (simulación sintética)"):
    ...
    if st.button("Ejecutar backtest"):
        ...
        st.success(f"Capital final: {final:.2f} MXN (Rendimiento: {(final-1000)/10:.2f}%)")
        if trades:
...
with col1:
    ...
    for sym in ["BTC","ETH"]:
        ...
        if strategy == "change":
            ...
            if len(hist) >= 15:
                ...
                sig = "🟢 COMPRAR" if rsi <= oversold else "🔴 VENDER" if rsi >= overbought else "⚪ MANTENER"
                valor = rsi
            else:
                valor = 0.0
                sig = "⏳ Cargando..."
    ...
    st.caption(f"Ciclo: {state['cycle']} | Estrategia: {strategy}")
with col2:
    st.subheader("📜 Operaciones")
    for ts, msg in reversed(state["trades"][-15:]):
        st.text(f"{ts.strftime('%H:%M:%S')} - {msg[:70]}")
    st.caption("Stop-loss, trailing, take-profit activos.")
